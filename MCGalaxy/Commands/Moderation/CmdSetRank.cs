...
namespace MCGalaxy.Commands.Moderation {
    public sealed class CmdSetRank : Command2 {
        ...
        public override CommandAlias[] Aliases {
            get {
                return new [] {
                    new CommandAlias("pr", "+up"), new CommandAlias("de", "-down"),
        ...
        internal static bool CanChangeRank(string name, Group curRank, Group newRank,
            ...
            if (newRank == banned) {
                p.Message("Use /ban to change a player's rank to {0}%S.", banned.ColoredName);
                return false;
            }
            if (curRank == banned) {
                p.Message("Use /unban to change a player's rank from %S{0}.", banned.ColoredName);
                return false;
            }
