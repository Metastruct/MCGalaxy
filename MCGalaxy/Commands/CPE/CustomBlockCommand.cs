    ...
    internal static class CustomBlockCommand {
        ...
        public static void Execute(Player p, string message, CommandData data, bool global, string cmd) {
            ...
            switch (parts[0]) {
                case "add":
                case "create":
                    AddHandler(p, parts, global, cmd);
                    break;
                case "copyall":
                case "copyfrom":
                    CopyAllHandler(p, parts, data, global, cmd);
                    break;
                case "copy":
                case "clone":
                case "duplicate":
                    CopyHandler(p, parts, data, global, cmd);
                    break;
                case "delete":
                case "remove":
                ...
                case "info":
                case "about":
                    InfoHandler(p, parts, global, cmd);
                    break;
                case "list":
                case "ids":
                    ListHandler(p, parts, global, cmd);
                    break;
                case "abort":
                    ...
                    break;
                case "edit":
                    EditHandler(p, parts, global, cmd);
                    break;
                default:
                    if (GetBD(p, global) != null)
        ...
        static bool ParseCoords(Player p, string parts, ref byte x, ref byte y, ref byte z) {
            ...
            x = (byte) tx;
            z = (byte) ty;
            y = (byte) tz; // blockdef files have z being height, we use y being height
            return true;
        }
        ...
        static Dictionary<string, string[]> helpSections = new Dictionary<string, string[]>() { { "name", new string[] { "Type the name for the block." } }, { "shape", new string[] { "Type '0' if the block is a cube, '1' if a sprite (e.g roses)." } }, { "blockslight", new string[] { "Type 'yes' if the block casts a shadow, 'no' if it doesn't." } }, { "fullbright", new string[] { "Type 'yes' if the block is fully lit (e.g. lava), 'no' if not." } },

            { "alltex", new string[] { "Type a number for all textures.", texLine } }, { "sidetex", new string[] { "Type a number for sides texture.", texLine } }, { "lefttex", new string[] { "Type a number for the left side texture.", texLine } }, { "righttex", new string[] { "Type a number for the right side texture.", texLine } }, { "fronttex", new string[] { "Type a number for the front side texture.", texLine } }, { "backtex", new string[] { "Type a number for the back side texture.", texLine } }, { "toptex", new string[] { "Type a number for the top texture.", texLine } }, { "bottomtex", new string[] { "Type a number for the bottom texture.", texLine } },

            {
            "min",
            new string[] {
            "Enter the three minimum coordinates of the cube in units (separated by spaces). 1 block = 16 units.",
            "Minimum coordinates for a normal block are &40 &20 &10."
            }
            }, {
            "max",
            new string[] {
            "Enter the three maximum coordinates of the cube in units (separated by spaces). 1 block = 16 units.",
            "Maximum coordinates for a normal block are &416 &216 &116."
            }
            }, {
            "collide",
            new string[] {
            "Type a number between '0' and '7' for collision type.",
            "0 - block is walk-through (e.g. air).",
            "1 - block is swim-through/climbable (e.g. rope).",
            "2 - block is solid (e.g. dirt).",
            "3 - block is solid, but slippery like ice",
            "4 - block is solid, but even slipperier than ice",
            "5 - block is swim-through like water",
            ...
            "speed",
            new string[] {
            "Type a number between '0.25' (25% speed) and '3.96' (396% speed).",
            "This speed is used when inside or walking on the block. Default speed is 1"
            }
            }, {
            "sound",
            new string[] {
            "Type a number between '0' and '9' for the sound played when walking on it and breaking.",
            ...
            "blockdraw",
            new string[] {
            "Enter the block's draw method.",
            "0 = Opaque, 1 = Transparent (Like glass)",
            "2 = Transparent (Like leaves), 3 = Translucent (Like ice), 4 = Gas (Like air)"
            }
            }, {
            "fogdensity",
            new string[] {
            "Enter the fog density for the block. 0 = No fog at all",
            "1 - 255 = Increasing density (e.g. water has 12, lava 255)"
            }
            }, { "fogcolor", new string[] { "Enter the fog color (hex color)" } }, {
            "fallback",
            new string[] {
            "Enter the fallback block (Block shown to players who can't see custom blocks).",
            "You can use any block name or block ID from the normal blocks."
            }
            }, {
            "order",
            new string[] {
            "Enter the position/order of this block in the inventory.",
