...
namespace MCGalaxy.Network {
    ...
    public sealed class IRCHandlers {
        ...
        void Listener_OnAction(UserInfo user, string channel, string description) {
            MessageInGame(user.Nick, string.Format("%I(IRC) * &R{0} &f{1}", user.Nick, description));
        }
        ...
        void Listener_OnPublic(UserInfo user, string channel, string message) {
            ...
            if (opchat) {
                ...
                Chat.MessageOps(string.Format("To Ops &f-%I(IRC) &R{0}&f- {1}", user.Nick,
        ...
        void Listener_OnNick(UserInfo user, string newNick) {
            ...
            MessageInGame(user.Nick, "%I(IRC) &R" + user.Nick + " %Sis now known as %R" + newNick);
        }
        ...
        void Listener_OnKick(UserInfo user, string channel, string kickee, string reason) {
            ...
            MessageInGame(user.Nick, string.Format("%I(IRC) &R{0} %Skicked &R{1} &f{2}", user.Nick, kickee, reason));
        }
